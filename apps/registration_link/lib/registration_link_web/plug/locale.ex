defmodule RegistrationLinkWeb.Plug.Locale do
  @moduledoc false
  ...
  def call(conn, default) do
    ...
    if is_ietf_tag && locale in RegistrationLinkWeb.Gettext.supported_locales do
  ...
  defp assign_locale!(conn, value) do
    ...
    Gettext.put_locale(RegistrationLinkWeb.Gettext, value)
    conn
    |> assign(:locale, value)
  end
  ...
  defp extract_locale(conn) do
    if Blank.present? conn.params["locale"] do
     locales = [conn.params["locale"] | extract_accept_language(conn)]
    else
    ...
    locales =
      locales
      |> Enum.filter(fn locale -> Enum.member?(MemberLinkWeb.Gettext.supported_locales, locale) end)
  end
