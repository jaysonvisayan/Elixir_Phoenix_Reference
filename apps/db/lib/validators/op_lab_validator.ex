defmodule Innerpeace.Db.Validators.OPLabValidator do
  ...
  def validate_authorization(authorization) do
    coverage = CoverageContext.get_coverage_by_code("OPL")
  ...
  defp compute_inner_limit(product_benefit, diagnosis_procedure, member, product) do
    benefit_limit = Enum.find(product_benefit.product_benefit_limits, &(String.contains?(&1.coverages, "OPL")))
  ...
  defp compute_inner_limit(diagnosis_procedure, member) do
    benefit_limit = Enum.find(diagnosis_procedure.product_benefit.benefit.benefit_limits, &(String.contains?(&1.coverages, "OPL")))
  ...
  defp filter_covered_procedures(product, procedures, member) do
    ...
    Enum.map(procedures, fn(procedure) ->
      ...
      benefit_limit = Enum.find(procedure.product_benefit.benefit_limits, &(String.contains?(&1.coverages, "OPL")))
  ...
  defp ceci_check_records([head | tails], data) do
    ...
    with true <- head.product.product_base == "Exclusion-based",
         true <- Enum.member?(coverages, op_lab.id)
    do
      gen_exclusions =
        product.product_exclusions
        |> Enum.map(&(&1.exclusion))
