export function modal_diagnosis_benefit(diagnosis_ids, csrf) {
    ...
    function updateDataTableSelectAllCtrl(table){
      ...
      var chkbox_select_all_diagnosis  = $('thead input[name="select_all_diagnosis"]', $table).get(0);

      // If none of the checkboxes are checked
      if($chkbox_checked.length === 0){
        chkbox_select_all_diagnosis.checked = false;
        if('indeterminate' in chkbox_select_all_diagnosis){
            chkbox_select_all_diagnosis.indeterminate = false;
        }

      // If all of the checkboxes are checked
      } else if ($chkbox_checked.length === $chkbox_all.length){
        chkbox_select_all_diagnosis.checked = true;
        if('indeterminate' in chkbox_select_all_diagnosis){
            chkbox_select_all_diagnosis.indeterminate = false;
        }
      ...
      // If some of the checkboxes are checked
        ...
        chkbox_select_all_diagnosis.checked = true;
        if('indeterminate' in chkbox_select_all_diagnosis){
            chkbox_select_all_diagnosis.indeterminate = true;
        }
      }
    }
      ...
      var table = $('#tbl_add_diagnosis').DataTable({
        ...
        "drawCallback": function (settings) {
          ...
          $(document).on('click', '#clear-all-checkbox', function(e) {
            ...
            $('input[name="select_all_diagnosis"]').prop("checked", false)
            $('#clear-select-message').remove()
          })
        },
        "renderer": 'semanticUI',
      ...
      $('thead input[name="select_all_diagnosis"]', table.table().container()).on('click', function(e){
...
function submit_diagnosis(rows, datas) {
  ...
  $('#diagnosis_loader').show();
    let is_all_selected = $('input[name="select_all_diagnosis"]').is(":checked")
