onmount('table[id="product_dental_risk_share_index"]', function() {
let product_id = $('#product_id').val()
let datatable = $('#product_dental_risk_share_index')
let ajax = `/web/products/dental_risk_share_index/data?product_id=${product_id}`
  ...
  function append_jump(page, pages){
    let results = $(`#product_dental_risk_share_index >tbody >tr`).length
    $('.table > tbody  > tr').each(function(){
      $(this).attr('style', 'height:50px')
    })
    $('#product_dental_risk_share_index_first').hide()
    $('#product_dental_risk_share_index_last').hide()
    $('#one_wide_rs').remove()
    $('#jump3_page').remove()
    $('#jump3_search').remove()
    $('#show3').remove()
    $('#product_dental_risk_share_index_length').find('label').before(`<span id="show3" style="margin-right:30px"> Showing ${results} results</span>`)
    $("#product_dental_risk_share_index_paginate").parent().parent().append(
    `<div class="one wide column inline field" id="one_wide_rs"></div>\
    <div class="right aligned two wide column inline field" id="jump3_page" style="padding-top:10px"> Jump to page: </div>\
    <div class="right aligned one wide column inline field" id="jump3_search" style="padding-left:0px">\
  ...
  $(document).on('change', '#jump3', function(){
    let page = $('#jump3_search > div > div > div.text').html()
    var table = datatable.DataTable();
  ...
  function remove_datatable(){
    $('#product_dental_risk_share_index_info').hide()
    $('#product_dental_risk_share_index_paginate').hide()
    $('#product_dental_risk_share_index_length').hide()
  }
