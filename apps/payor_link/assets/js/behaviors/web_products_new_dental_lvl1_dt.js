onmount('div[id="product_dental_plan"]', function () {
  ...
  $('#btn_add_benefit').unbind('click').click(function () {
    ...
    // Add Benefit Modal Datatable
      ...
      "language": {
        ...
        let datatables = $('#tbl_benefit_dt_table')

        // $('#Searchbenefit').find('input[type="search"]').keypress(function() {
        $(document).on('keyup', '#Searchbenefit', function() {
    ...
    var row_data = {
      ...
      [
        ...
        `<span class="${firstColData}"> ${firstColData} </span>`,
				...
				`<div class="ui dropdown" tabindex="0">\
                   ...
                   style="color:#00B24F" \
				        ...
				        <span class="${firstColData}_ben_rows selected_ben_rows hidden">${rowId}|${fourthColData}|${fifthColData}</span> \
					     ...
					     <a href="#!" class="remove_package" role="${rowId}" code="${firstColData}"> Remove</a><span class="selected_procedure_id hidden">${rowId}</span>\
				      </div>\
            </div>\
        </div>`
      ]
  ...
  // Add Benefit
	$(document).unbind('click').on('click', '#btn_submit_benefit', function() {
		...
		if(rows_selected.length > 0){
          ...
          if(response === "true")
            ...
            function edit_limit() {
              $('.edit_limit').unbind('click').click(function() {
                // TODO
                ...
                $('.modal.limit').modal({
                  ...
                  onApprove: () =>{
                    ...
                    if (updated_limit_amount != "" && updated_limit_amount != 0) {
                    ...
                    $.each(row_datas, function(x, y) {
                      if(y.code == b_code)
                      {
                        y.limit_type = updated_limit_type;
                        y.limit_amount = updated_limit_amount;
                      }
                    })
                    ...
                    $.each(row_datas, function(x, y) {
                      var data = [
                        `<span class="${y.code}"> ${y.code} </span>`,
                        `<span class="${y.code}${y.name}"> ${y.name} </span>`,
                        `<span class="${y.code}${y.no_of_cdt}"> ${y.no_of_cdt} </span>`,
                        `<span class="${y.code}${y.limit_type}"> ${y.limit_type} </span>`,
                        `<span class="${y.code}${y.limit_amount}"> ${y.limit_amount} </span>`,
                        `<div class="ui dropdown" tabindex="0">\
                            <i class="ellipsis vertical icon"></i>\
                            <div class="menu transition hidden">\
                              <div class="item clickable-row edit_limit" data-href="#" \
                                  ...
                                  code="${y.code}" \
                                  name="${y.name}" \
                                  limit_type="${y.limit_type}" \
                                  limit_amount="${y.limit_amount}" \
                                  >Edit limit\
                                <span class="selected_procedure_id hidden">${y.id}</span> \
                                <span class="${y.code}_ben_rows selected_ben_rows hidden">${y.id}|${y.limit_type}|${y.limit_amount}</span> \
                              </div>\
                              <div class="item clickable-row">\
                              <a href="#!" class="remove_package" role="${y.id}" code="${y.code}"> Remove</a><span class="selected_procedure_id hidden">${y.id}</span>\
                              </div>\
                            </div>\
                        </div>`
                    ]

                    edited_datas.push(data)
                  })

                    let dt = $('#tbl_benefit_dt_table').DataTable();
                    dt.clear().draw();

                    $(edited_datas).each(function(x, y) {
                      dt.row.add(y).draw(false)
                    })
                    $('.modal.procedure').modal('hide');
                    $('input[name="is_valid_procedure"]').val("true")
          ...
          function remove_package() {
            $('.remove_package').unbind().click(function() {
              let dt = $('#tbl_benefit_dt_table').DataTable();
    ...
    function remove_row(row, dt, row2){
      ...
      swal({
        ...
        let new_row_datas = row_datas.filter((value, index, array) => {
     ...
     // $('#tbl_benefit_dt_table tbody').unbind('keypress').on('keypress', '#Searchbenefit', function(e){
     //  alert(1)
  ...
  $('input[name="product[benefit_ids][]"]').val(Array.from(new Set(selected_benefit_ids)))
    ...
    let validate_form = $('#dental_form').form('validate form')
    if (validate_form){
      $('#dental_form').submit()
    }
  })
  ...
  $(document).on('change', '#jump', function(){
    let datatable = $('#dt-dental-benefit')
    let page = $('#jump_search > div > div > div.text').html()
    var table = datatable.DataTable();
  ...
  $(document).on('change', '#jump2', function(){
    let datatable = $('#tbl_benefit_dt_table')
...
onmount('#tbl_benefit_dt_table', function () {
  $('#tbl_benefit_dt_table').dataTable({
    lengthMenu: [
      [10, 50, 100, 500, 1000],
      [10, 50, 100, 500, 1000]
    ]  ,
  ...
  $('.append-search').append('<div class="dataTables_filter ui input left icon">\<input type="search" class="" placeholder="Search" id="Searchbenefit">\<i class="search icon"></i></div>')

  // Search Benefit
  // $('#Searchbenefit').unbind('keypress').on('keypress', function(e){
  //   var value = $(this).val().toLowerCase();
  //   $("#tbl_benefit_dt_table tr").filter(function() {
  //     $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
  //   });

  //   // var input, filter, table, tr, td, i;
  //   // input = document.getElementById("Searchbenefit");
  //   // filter = input.value.toUpperCase();
  //   // table = document.getElementById("tbl_benefit_dt_table");
...
function add_search(table){
  ...
  if(value != 1){
    ...
    $(`#${id}_filter`).find('input[type="search"]').attr("id", `Searchbenefit`)
