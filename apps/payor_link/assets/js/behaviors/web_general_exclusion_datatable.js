...
export function ExclusionProcedure(table, dataSrc, token, data, url) {
   let dTable = '#' + table
   let tbl_length = dTable + '_length'
   let tbl_wrapper = dTable + '_wrapper'
...
export function append_jump(table, tbl_length, tbl_wrapper, page, pages){
  // let results = $(`select[name="${table + '_length'}"]`).val()
    ...
    let resulta2;
    let diagnosis_search_id = $(tbl_wrapper)
  ...
  $(tbl_length)
    .find('label')
    .before(`<span class="show" style="margin-right:30px"> Showing ${resulta2} results</span>`)
