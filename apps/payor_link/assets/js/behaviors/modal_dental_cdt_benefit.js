export function modal_cdt_benefit(diagnosis_ids, csrf) {
    let type = ""
    let datatable = $('#tbl_add_cdt')
    ...
    function updateDataTableSelectAllCtrl(table){
      ...
      var chkbox_select_all_cdt  = $('thead input[name="select_all_cdt"]', $table).get(0);

      // If none of the checkboxes are checked
      if($chkbox_checked.length === 0){
        chkbox_select_all_cdt.checked = false;
        if('indeterminate' in chkbox_select_all_cdt){
            chkbox_select_all_cdt.indeterminate = false;
        }

      // If all of the checkboxes are checked
      } else if ($chkbox_checked.length === $chkbox_all.length){
        chkbox_select_all_cdt.checked = true;
        if('indeterminate' in chkbox_select_all_cdt){
            chkbox_select_all_cdt.indeterminate = false;
        }
      ...
      // If some of the checkboxes are checked
        ...
        chkbox_select_all_cdt.checked = true;
        if('indeterminate' in chkbox_select_all_cdt){
            chkbox_select_all_cdt.indeterminate = true;
        }
      }
    }
      ...
      var table = $('#tbl_add_cdt').DataTable({
        "dom":
        "<'ui grid'"+
          "<'row'"+
            "<'four wide column'f>"+
        ...
        "ajax": $.fn.dataTable.dt_timeout(
          `/web/benefits/load_diagnosis?diagnosis_ids=${diagnosis_ids}&type=${type}&is_all_selected=${ $('input[name="select_all_cdt"').is(":checked") }`,
          csrf
        ),
        'columnDefs': [{
            'targets': 0,
        ...
        "drawCallback": function (settings) {
          ...
          $('button[id="btn_submit_cdt"]').remove()
          $('.test').append('<button class=" send-rtp ui left floated primary button close" type="submit" id="btn_submit_cdt">Add</button>')
            ...
            let type = $('#diagnosis_type_dropdown').dropdown('get value')
             ...
             $.ajax({
               ...
               success: function(response) {
                ...
                <p>All ${response.data.length + 10} CDT are selected. <a id="clear-all-checkbox"> <span class="green"> <u> Clear selection </u> </span> </a> \
                </p></div>`)
               }})
          })
          ...
          $(document).on('click', '#clear-all-checkbox', function(e) {
            ...
            $('input[name="select_all_cdt"]').prop("checked", false)
            $('#clear-select-message').remove()
          })
        },
        "renderer": 'semanticUI',
      ...
      $('#tbl_add_cdt_wrapper').find('.dataTables_filter').addClass('ui left icon input')
      $('#tbl_add_cdt_wrapper').find('.dataTables_filter').find('input[type="search"]').unwrap('Search')
      $('#tbl_add_cdt_wrapper').find('.dataTables_filter').find('input[type="search"]').attr("placeholder", `Search`)
      $('#tbl_add_cdt_wrapper').find('.dataTables_filter').find('input[type="search"]').append(`</div>`)

      $(document).unbind('change').on('change', '.ui.fluid.dropdown.cdt', function(){
      ...
      let cdt2_search_id = $('#tbl_add_cdt_wrapper')
      ...
      if (results = cdt2_search_id.find('.table > tbody  > tr').html().includes("matched your search") || cdt2_search_id.find('.table > tbody  > tr').html().includes("found")) {
        ...
        cdt2_search_id.find(".dataTables_paginate.paging_simple_numbers").parent().parent().append(
          `<div class="right aligned two wide column inline field jump-to-page" style="padding-top: 0.8rem;"> Jump to page: </div>\
            ...
            <select class="ui fluid dropdown cdt" id="jump_cdt">\
            </select>\
            </div>`
        )
        var select = $('#jump_cdt')
        ...
        cdt2_search_id.find('.ui.fluid.dropdown.cdt').dropdown()
        cdt2_search_id.find('.ui.fluid.dropdown.cdt.selection').css("width", "6rem")
        cdt2_search_id.find('.ui.fluid.dropdown.cdt.selection').css("margin-left", "-1.2rem")
        cdt2_search_id.find('#jump_cdt').dropdown('set selected', page + 1)
      }

      // Handle click on checkbox
      $('#tbl_add_cdt tbody').on('click', 'input[type="checkbox"]', function(e){
        ...
        var datas =
          [
            ...
            `<a href="#!" id="${CDTId}" class="remove_cdt"><u style="color: gray">Remove</u></a><input type="hidden" name="benefit[diagnosis_ids][]" value="${CDTId}">`
          ]
      ...
      $('#tbl_add_cdt').on('click', 'tbody td, thead th:first-child', function(e){
        $(this).parent().find('label[type="text"]').trigger('click');
      });

      // Handle click on "Select all" control
      $('thead input[name="select_all_cdt"]', table.table().container()).on('click', function(e){
        let id = "tbl_add_cdt"
        ...
        if(this.checked){
            $('#clear-select-message').remove()
            $('#tbl_add_cdt tbody input[type="checkbox"]:not(:checked)').trigger('click');
            $('.dt-append').append(`<div class="ui attached message" id="clear-select-message" style="background-color: #fff8ea;"> \
            ...
            $('#clear-select-message').remove()
            $('#tbl_add_cdt tbody input[type="checkbox"]:checked').trigger('click');
        }
...
export function add_search(table){
  ...
  const initializeDeleteDiagnosis = _ => {
    $(document).on('click', '.remove_cdt', function(e) {
      let id = $(this).attr('id')
...
function submit_diagnosis(rows, datas) {
  ...
  $('button[id="btn_submit_cdt"]').unbind('click').click(function() {
    $('#cdt_loader').show();
    let is_all_selected = $('input[name="select_all_cdt"]').is(":checked")

    let checked_cdt = $('input[class="diagnosis_chkbx"]:checked')
    if(rows.length > 0){
      let submit_btn = $(this)
      submit_btn.html('<span class="ui active tiny centered inline loader"></span>')
      let dt = $('#tbl_cdt').DataTable();

      let datas2 = removeDuplicateUsingFilter(datas)
      $('#cdt_loader').removeClass('hidden')
      $(removeDuplicateUsingFilter(datas)).each(function(x, y) {
        dt.row.add(y).draw(false)
      })
      $('#cdt_loader').addClass('hidden')
      $('.modal.cdt').modal('hide');
