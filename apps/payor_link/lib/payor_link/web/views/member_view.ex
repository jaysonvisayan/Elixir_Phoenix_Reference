defmodule Innerpeace.PayorLink.Web.MemberView do
  ...
  def check_age(package_payor_procedures, member) do
    ...
    cond do
      is_nil(age_from) or is_nil(age_to) ->
        true
      age_from <= age(member.birthdate) and age(member.birthdate) <= age_to ->
        true
      true ->
        {:invalid_age}
    end
  ...
  #   member_age = age(member.birthdate)
  ...
  # defp check_product_packages(member, product) do
  #   gender = member.gender
  #   asd = if not is_nil(product.genders) do
  #     if Enum.member?(product.genders, gender) do
  #       true
  #     else
  #       {:not_valid_gender}
  #     end
  ...
  def authorization_practitioner(authorization, coverage) do
    case coverage do
      "op consult" ->
        ...
        aps
        |> Enum.into([], &(Enum.join([
          &1.practitioner_specialization.practitioner.first_name,
          &1.practitioner_specialization.practitioner.last_name], " ")))
        |> Enum.join(", ")
      "op laboratory" ->
        "N/A"
      "acu" ->
  ...
  def render("member.json", %{member: member}) do
    %{
      id: member.id,
      name: Enum.join([member.first_name, member.middle_name, member.last_name], " "),
  ...
  def check_created_by("payorlink", auth), do: validate_payorlink(auth.created_by)
  def check_created_by("providerlink", auth), do: validate_providerlink(auth.requested_by)
  def check_created_by("Accountlink", _auth), do: "Accountlink"
  def check_created_by("Memberlink", _auth), do: "Memberlink"
  def check_created_by(_, _), do: "Invalid Created By"

  def validate_payorlink(nil), do: "Invalid Created By"
  def validate_payorlink([]), do: "Invalid Created By"
  def validate_payorlink(created_by), do: created_by.username

  def validate_providerlink(nil), do: "Invalid Created By"
  def validate_providerlink(""), do: "Invalid Created By"
  def validate_providerlink(requested_by), do: requested_by

  def check_processed_by("payorlink", auth), do: validate_payorlink(auth.created_by)
  def check_processed_by("Accountlink", auth), do: validate_payorlink(auth.created_by)
  def check_processed_by("Memberlink", auth), do: validate_payorlink(auth.created_by)
  def check_processed_by("providerlink", _), do: "Sonny Medina"
  def check_processed_by(_, _), do: "Invalid Requested By"
