...
onmount('div[id="file"]', function(){
  $('button[id="upload"]').on('click', function(){
    // Get file input name
    let name =  $(this).attr("file")
    const file_input_selector = $('input[name="kyc['+ name +']"]')
    ...
    file_input_selector.on('click', function(){
      $(this).on('change', function(){
        ...
        if($(this)[0].files[0].size <= 1024 * 1024 * 5){
          // Type checker
          if(name == "front_side" || name == "back_side"){
...
onmount('div[id="evoucher_details_file"]', function(){
  $('label[id="upload"]').on('click', function(){
    // Get file input name
    let name =  $(this).attr("file")
    const extensionListsImage  = ['jpg', 'jpeg', 'png'];
    const extensionListsFile  = ['jpg', 'jpeg', 'png'];
    //$('#secondary_id').click()

    $(`#${name}`).on('change', function(){
      if ($(this).val() == '') {

        $('span[name="'+ name +'"]').text('Upload' + name)
