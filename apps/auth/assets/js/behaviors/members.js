...
onmount('div[name="MemberGeneral"]', function () {
  ...
  $.ajax({
    ...
    success: function(response){
      ...
      $.each(members, function(index, member) {
        if (member.type == "Principal" && member.id != member_id && member.status == "Active") {
      ...
      for (let dependent of selected_principal.dependents) {
        if (dependent.id == member_id){
          ...
          $('.edit_skip_icon').on('click', function(){
            console.log($(this).closest('tr').find('#table_relationship').attr('first_name'))
  ...
  $('#submitGeneral').on('click', function() {
    if ($('input[name="member[type]"]:checked').val() == "Dependent" && $('#principalID').val() != "" && $('#relationship').val() != "") {
  ...
  function validate_hierarchy_skipping() {
...
onmount('input[id="deleteMemberProductSuccess"]', function() {
  alertify.success(`<i class="close icon"></i><p>Successfully deleted plan/s</p>`);
});
...
onmount('div[id="member_batch_upload"]', function() {
  ...
  $('#batch_enrollment_template').click(function(){
    let csv = `Employee No,Member Type,Relationship,Effective Date,Expiry Date,First Name,Middle Name,Last Name,Suffix,Gender,Civil Status,Birthdate,Mobile,Email,Date Hired,Regularization Date,Address,City,Product Code,For Card Issuance,Tin No,Philhealth,Philhealth No\n(Required) Employee No. of the Principal or guardian,(Required) Member type of member: Principal Dependent Guardian,(Required for dependents) Spouse Child Parent Sibling,(Required) Date must be in mm/dd/yyyy format,(Required) Date must be in mm/dd/yyyy format,"(Required) Must be containing 150 characters consist of letters and dot (.) comma (,) and hyphen (-)","(Optional) Must be containing 150 characters consist of letters and dot (.) comma (,) and hyphen (-)","(Required) Must be containing 150 characters consist of letters and dot (.) comma (,) and hyphen (-)","(Optional) Must be containing 10 characters consist of letters and dot (.) comma (,) and hyphen (-)",(Required) Male or Female,(Required) Status: Single Single Parent Married Widow / Widower Annulled Separated,(Required) Date must be in mm/dd/yyyy format,(Required For Principal and guardian) Must be 11 digits only. Format must be 09********,"(Required For Principal and guardian) Must contain alphanumeric characters with special characters dot (.)  hypen (-), at (@) and underscore(_)",(Required For Principal) Date must be in mm/dd/yyyy format,(Required For Principal) Date must be in mm/dd/yyyy format,(Optional) Address of the member,(Optional) City Address of the member,(Required) Product code of the product of the member,(Required) Yes No,(Optional) Must be containing twelve(12) numeric characters only,(Required) Required to file Optional to file Not Covered,(Optional) Must be containing twelve(12) numeric characters only`
